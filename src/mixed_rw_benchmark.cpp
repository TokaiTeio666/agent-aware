...
struct Args {
  ...
  double recall_max_samples_per_sec = 0.0;
...
struct ReadBreakdownSamples {
  std::vector<double> dynamic_snapshot_ms;
  std::vector<double> base_search_ms;
  std::vector<double> latest_record_lookup_ms;
  std::vector<double> delta_search_ms;
  std::vector<double> merge_ms;
  std::vector<double> exact_recall_ms;
  std::vector<double> search_mutex_wait_ms;
  std::vector<double> page_read_wait_ms;
};

struct ReadBreakdownSummary {
  Percentiles dynamic_snapshot;
  Percentiles base_search;
  Percentiles latest_record_lookup;
  Percentiles delta_search;
  Percentiles merge;
  Percentiles exact_recall;
  Percentiles search_mutex_wait;
  Percentiles page_read_wait;
};
...
struct ScenarioResult {
  ...
  std::string dirty_status = "unknown";
  std::string index_path;
  std::string data_path;
  std::string dynamic_dir;
  ...
  double page_read_wait_ms_per_read = 0.0;
  double search_mutex_wait_ms_per_read = 0.0;
  ReadBreakdownSummary read_breakdown;
...
struct ThreadMetrics {
  ...
  ReadBreakdownSamples read_breakdown;
};
...
void print_help() {
  std::cout
      ...
      << "  --recall_max_samples_per_sec R  Optional global recall sample rate cap\n"
...
Args parse_args(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    } else if (name == "--recall_max_samples_per_sec") {
      args.recall_max_samples_per_sec =
  ...
  if (args.duration_sec < 0.0 || args.recall_sample_rate < 0.0 ||
      ...
      args.recall_max_samples_per_sec < 0.0) {
    throw std::runtime_error(
        "duration_sec and recall_max_samples_per_sec must be non-negative; recall_sample_rate must be in [0,1]");
  }
  return args;
}
...
std::string capture_command(const char* command) {
  ...
  output = trim_ascii(std::move(output));
  return output.empty() ? "unknown" : output;
}

std::string git_commit() {
  return capture_command("git rev-parse --short HEAD 2>/dev/null");
}

std::string git_dirty_status() {
  const std::string status =
      capture_command("git status --short 2>/dev/null");
  return status == "unknown" || status.empty() ? "clean" : "dirty";
}
...
void append_read_timing(ThreadMetrics& metrics,
                        const agent_aware::EngineSearchStats& stats) {
  metrics.graph_stats.node_reads += stats.graph.node_reads;
  metrics.graph_stats.page_requests += stats.graph.page_requests;
  metrics.graph_stats.cache_hits += stats.graph.cache_hits;
  metrics.graph_stats.pinned_hits += stats.graph.pinned_hits;
  metrics.graph_stats.pending_hits += stats.graph.pending_hits;
  metrics.graph_stats.demand_reads += stats.graph.demand_reads;
  metrics.graph_stats.prefetch_reads += stats.graph.prefetch_reads;
  metrics.graph_stats.duplicate_skipped += stats.graph.duplicate_skipped;
  metrics.graph_stats.submitted_reads += stats.graph.submitted_reads;
  metrics.graph_stats.completed_reads += stats.graph.completed_reads;
  metrics.graph_stats.page_cache_hits += stats.graph.page_cache_hits;
  metrics.graph_stats.page_cache_misses += stats.graph.page_cache_misses;
  metrics.graph_stats.demand_read_wait_us += stats.graph.demand_read_wait_us;
  metrics.graph_stats.search_mutex_wait_us += stats.graph.search_mutex_wait_us;
  metrics.read_breakdown.dynamic_snapshot_ms.push_back(
      stats.timing.dynamic_snapshot_ms);
  metrics.read_breakdown.base_search_ms.push_back(stats.timing.base_search_ms);
  metrics.read_breakdown.latest_record_lookup_ms.push_back(
      stats.timing.latest_record_lookup_ms);
  metrics.read_breakdown.delta_search_ms.push_back(
      stats.timing.delta_search_ms);
  metrics.read_breakdown.merge_ms.push_back(stats.timing.merge_ms);
  metrics.read_breakdown.search_mutex_wait_ms.push_back(
      stats.graph.search_mutex_wait_us / 1000.0);
  metrics.read_breakdown.page_read_wait_ms.push_back(
      stats.graph.demand_read_wait_us / 1000.0);
}

void append_exact_recall_timing(ThreadMetrics& metrics, double elapsed_ms) {
  metrics.read_breakdown.exact_recall_ms.push_back(elapsed_ms);
}

void merge_samples(std::vector<double>& into, std::vector<double>&& from) {
  into.insert(into.end(), from.begin(), from.end());
}

void merge_read_breakdown(ReadBreakdownSamples& into,
                          ReadBreakdownSamples&& from) {
  merge_samples(into.dynamic_snapshot_ms, std::move(from.dynamic_snapshot_ms));
  merge_samples(into.base_search_ms, std::move(from.base_search_ms));
  merge_samples(into.latest_record_lookup_ms,
                std::move(from.latest_record_lookup_ms));
  merge_samples(into.delta_search_ms, std::move(from.delta_search_ms));
  merge_samples(into.merge_ms, std::move(from.merge_ms));
  merge_samples(into.exact_recall_ms, std::move(from.exact_recall_ms));
  merge_samples(into.search_mutex_wait_ms,
                std::move(from.search_mutex_wait_ms));
  merge_samples(into.page_read_wait_ms, std::move(from.page_read_wait_ms));
}

ReadBreakdownSummary summarize_breakdown(ReadBreakdownSamples&& samples) {
  ReadBreakdownSummary summary;
  summary.dynamic_snapshot = summarize(std::move(samples.dynamic_snapshot_ms));
  summary.base_search = summarize(std::move(samples.base_search_ms));
  summary.latest_record_lookup =
      summarize(std::move(samples.latest_record_lookup_ms));
  summary.delta_search = summarize(std::move(samples.delta_search_ms));
  summary.merge = summarize(std::move(samples.merge_ms));
  summary.exact_recall = summarize(std::move(samples.exact_recall_ms));
  summary.search_mutex_wait =
      summarize(std::move(samples.search_mutex_wait_ms));
  summary.page_read_wait = summarize(std::move(samples.page_read_wait_ms));
  return summary;
}
...
bool allow_recall_sample(const Args& args,
                         std::atomic<std::int64_t>& next_recall_ns) {
  if (args.recall_max_samples_per_sec <= 0.0) {
    return true;
  }
  const auto interval_ns = static_cast<std::int64_t>(
      1000000000.0 / args.recall_max_samples_per_sec);
  const auto now = steady_now_ns();
  auto expected = next_recall_ns.load(std::memory_order_relaxed);
  while (now >= expected) {
    const auto desired = now + std::max<std::int64_t>(1, interval_ns);
    if (next_recall_ns.compare_exchange_weak(
...
void record_recall_sample(
  ...
  const auto recall_start = Clock::now();
  ...
  append_exact_recall_timing(metrics, elapsed_ms(recall_start, Clock::now()));
}
...
void merge_thread_metrics(ThreadMetrics& into, ThreadMetrics&& from) {
  ...
  into.graph_stats.cache_hits += from.graph_stats.cache_hits;
  into.graph_stats.pinned_hits += from.graph_stats.pinned_hits;
  into.graph_stats.pending_hits += from.graph_stats.pending_hits;
  into.graph_stats.demand_reads += from.graph_stats.demand_reads;
  into.graph_stats.prefetch_reads += from.graph_stats.prefetch_reads;
  into.graph_stats.duplicate_skipped += from.graph_stats.duplicate_skipped;
  into.graph_stats.submitted_reads += from.graph_stats.submitted_reads;
  into.graph_stats.completed_reads += from.graph_stats.completed_reads;
  into.graph_stats.page_cache_hits += from.graph_stats.page_cache_hits;
  into.graph_stats.page_cache_misses += from.graph_stats.page_cache_misses;
  into.graph_stats.demand_read_wait_us += from.graph_stats.demand_read_wait_us;
  into.graph_stats.search_mutex_wait_us +=
      from.graph_stats.search_mutex_wait_us;
  merge_read_breakdown(into.read_breakdown, std::move(from.read_breakdown));
}
...
ScenarioResult finalize_result(
  ...
  result.search_width = args.search_width;
  result.beam_width = args.beam_width;
  result.entry_count = args.entry_count;
  result.enable_compaction = args.enable_compaction;
  result.compaction_background = args.compaction_background;
  result.recall_max_samples_per_sec = args.recall_max_samples_per_sec;
  result.git_commit = git_commit();
  result.dirty_status = git_dirty_status();
  result.index_path = args.index_path;
  result.data_path = args.data_path;
  result.dynamic_dir = scenario_dir.string();
  ...
  result.read_breakdown = summarize_breakdown(std::move(metrics.read_breakdown));
  ...
  result.graph_reads_per_read =
      metrics.read_ops == 0 ? 0.0
                            : static_cast<double>(
                                  metrics.graph_stats.submitted_reads) /
                                  static_cast<double>(metrics.read_ops);
  result.cache_hit_rate =
      metrics.graph_stats.page_requests == 0
          ? 0.0
          : static_cast<double>(metrics.graph_stats.cache_hits) /
                static_cast<double>(metrics.graph_stats.page_requests);
  result.page_read_wait_ms_per_read =
      metrics.read_ops == 0
          ? 0.0
          : (metrics.graph_stats.demand_read_wait_us / 1000.0) /
                static_cast<double>(metrics.read_ops);
  result.search_mutex_wait_ms_per_read =
      metrics.read_ops == 0
          ? 0.0
          : (metrics.graph_stats.search_mutex_wait_us / 1000.0) /
                static_cast<double>(metrics.read_ops);
...
ScenarioResult run_sequential_scenario(
  ...
  std::atomic<std::int64_t> next_recall_ns{0};
  ...
  for (std::size_t op = 0; op < args.num_operations; ++op) {
    ...
    if (do_read) {
      ...
      if (engine) {
        ...
        append_read_timing(metrics, engine_result.stats);
      } else {
      ...
      if (sample_pick(rng) <= args.recall_sample_rate &&
          allow_recall_sample(args, next_recall_ns)) {
...
void write_breakdown_percentiles(std::ofstream& output,
                                 const std::string& prefix,
                                 const Percentiles& values,
                                 bool trailing_comma) {
  output << "        \"" << prefix << "_avg_ms\": " << values.avg << ",\n"
         << "        \"" << prefix << "_p50_ms\": " << values.p50 << ",\n"
         << "        \"" << prefix << "_p95_ms\": " << values.p95 << ",\n"
         << "        \"" << prefix << "_p99_ms\": " << values.p99 << ",\n"
         << "        \"" << prefix << "_p999_ms\": " << values.p999
         << (trailing_comma ? ",\n" : "\n");
}
...
void write_json(const std::filesystem::path& output_path,
  ...
  for (std::size_t i = 0; i < results.size(); ++i) {
    ...
    output << "    {\n"
           ...
           << "      \"benchmark_config\": {\n"
           << "        \"data_path\": \"" << json_escape(result.data_path)
           << "\",\n"
           << "        \"index_path\": \"" << json_escape(result.index_path)
           << "\",\n"
           << "        \"dynamic_dir\": \"" << json_escape(result.dynamic_dir)
           << "\",\n"
           << "        \"topk\": " << result.topk << ",\n"
           << "        \"search_width\": " << result.search_width << ",\n"
           << "        \"beam_width\": " << result.beam_width << ",\n"
           << "        \"entry_count\": " << result.entry_count << ",\n"
           << "        \"recall_sample_rate\": "
           << result.recall_sample_rate << ",\n"
           << "        \"recall_max_samples_per_sec\": "
           << result.recall_max_samples_per_sec << ",\n"
           << "        \"enable_compaction\": "
           << (result.enable_compaction ? "true" : "false") << ",\n"
           << "        \"compaction_background\": "
           << (result.compaction_background ? "true" : "false") << ",\n"
           << "        \"git_commit\": \"" << json_escape(result.git_commit)
           << "\",\n"
           << "        \"dirty_status\": \""
           << json_escape(result.dirty_status) << "\"\n"
           << "      },\n"
           ...
           << "      \"page_read_wait_ms_per_read\": "
           << result.page_read_wait_ms_per_read << ",\n"
           << "      \"search_mutex_wait_ms_per_read\": "
           << result.search_mutex_wait_ms_per_read << ",\n"
           << "      \"read_breakdown\": {\n";
    write_breakdown_percentiles(output, "dynamic_snapshot",
                                result.read_breakdown.dynamic_snapshot, true);
    write_breakdown_percentiles(output, "base_search",
                                result.read_breakdown.base_search, true);
    write_breakdown_percentiles(
        output, "latest_record_lookup",
        result.read_breakdown.latest_record_lookup, true);
    write_breakdown_percentiles(output, "delta_search",
                                result.read_breakdown.delta_search, true);
    write_breakdown_percentiles(output, "merge", result.read_breakdown.merge,
                                true);
    write_breakdown_percentiles(output, "exact_recall",
                                result.read_breakdown.exact_recall, true);
    write_breakdown_percentiles(
        output, "search_mutex_wait",
        result.read_breakdown.search_mutex_wait, true);
    write_breakdown_percentiles(output, "page_read_wait",
                                result.read_breakdown.page_read_wait, false);
    output << "      },\n"
