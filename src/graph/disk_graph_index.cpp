...
void PackedDiskGraphIndex::maybe_issue_prefetch(SearchState& state) const {
  ...
  if (!state.session.frontier_prefetch_enabled() || state.candidates.empty()) {
    return;
  }

  auto frontier = state.candidates;
  std::vector<std::uint32_t> page_ids;
  page_ids.reserve(state.session.prefetch_width());
  while (!frontier.empty() &&
         page_ids.size() < state.session.prefetch_width()) {
  ...
  state.session.submit_prefetch(page_ids);
}
