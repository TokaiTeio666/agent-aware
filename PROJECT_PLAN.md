...
```mermaid
flowchart TB
    A["Agent / Benchmark 请求"] --> B["查询与写入调度层"]
    B --> C["DiskANN/Vamana 图导航"]
    C --> D["PQ ADC 近似过滤"]
    D --> E["候选节点批处理"]
    E --> F["Graph-Aware 2Q BufferPool"]
    F --> G["O_DIRECT 4KB Node Record"]
    G --> H["SSD 全精度向量"]
    E --> I["io_uring 下一跳预取"]
    I --> G
...
写路径采用 LSM-Tree 思想，将随机写转为顺序追加写：

```mermaid
flowchart LR
    A["insert/update/delete"] --> B["WAL 追加写"]
    B --> C["Active MemTable"]
    C --> D{"MemTable 满?"}
    D -- "否" --> E["返回写入成功"]
    D -- "是" --> F["Immutable MemTable"]
    F --> G["Flush SSTable"]
    G --> H["后台 Compaction"]
    A --> I["图索引增量插入/更新"]
```

读回顺序为 Active MemTable、Immutable MemTable、SSTable，保证新数据优先。删除操作通过 tombstone 表示，后台 Compaction 清理过期版本。
