...
缓存与零拷贝的关键读路径优化已经部分落地：当前 packed graph 查询路径包含 page cache 命中/淘汰/同页复用统计，支持 `compute_distance_direct`，并能输出 cache、direct distance、page dedup 等指标。独立 BufferPool 管理器、严格 pin/unpin 生命周期和更系统的 2Q 对照实验仍可作为后续工程化增强。
...
本设计主题的目标不是继续扩大搜索宽度，而是减少重复 I/O、保护高价值图节点、降低距离计算路径中的拷贝成本，从而提升 SSD 图搜索的缓存命中率与尾延迟表现。
...
因此，本设计的核心思路是让缓存理解图结构，让距离计算尽量贴近 page 原始数据，让并发访问保持正确性。

## 核心目标

缓存与零拷贝优化需要完成：

1. Graph-Aware 2Q 缓存：将图入度引入缓存淘汰策略，保护 hub 节点，提升随机图遍历缓存命中率。
2. pin/unpin 页面保护：搜索过程中临时固定关键 page，避免热点节点在一次 query 内被误淘汰。
3. `compute_distance_direct`：直接基于 page 内向量数据计算距离，减少 page 到临时 vector 的内存拷贝。
4. 线程安全缓存接口：支持多线程 query 压测，避免缓存结构在并发场景下出现数据竞争。
...
如果后续拆出独立缓存模块，建议将缓存实现与图搜索逻辑解耦，并保留统一 get/put/pin/unpin/stat 接口。
...
1. `A1in`：新进入缓存的页面队列。
2. `Am`：多次命中的主缓存队列。
3. `A1out`：被淘汰页面的 ghost 队列，用于判断页面是否值得重新进入主缓存。

本项目在此基础上加入图结构信息：

1. 每个节点记录 `in_degree` 或近似访问重要性。
2. 高入度节点被视为 hub 节点。
3. hub 节点命中后优先进入或保留在 `Am`。
4. 淘汰时优先淘汰低入度、低访问频率、未 pin 的页面。
5. 支持统计 page hit、miss、eviction、pin count、hub hit ratio。
...
1. `access_score`：历史访问频率。
2. `in_degree`：图入度或 hub 权重。
3. `recent_hit`：近期是否命中。
4. `cold_age`：距离上次访问的时间。
5. `alpha/beta/gamma`：可通过实验调参。
...
2. 减少临时对象构造。
3. 复用已有 SIMD 距离计算函数。
4. 保持 page layout 与 `O_DIRECT` 对齐要求兼容。
...
| 实现基础 2Q | 支持 A1in、Am、A1out | 覆盖命中、淘汰、ghost hit |
...
| 线程安全改造 | mutex/shared_mutex/分片锁 | 压力验证无数据竞争 |
...
9. 最终报告需要回答 Graph-Aware 2Q 是否优于普通 LRU / 2Q。
10. 最终报告需要说明 pin/unpin 是否能保护 query 内关键 page 且没有线程安全问题。
11. 最终报告需要说明 `compute_distance_direct` 是否减少拷贝开销并保持 Recall 不变。
