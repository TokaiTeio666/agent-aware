# Codex Prompt：完善 beam_width 与 io_uring 批量读取

## 当前实现状态（2026-06-21）

该文档原本是实现提示词，现在作为 `beam_width + io_uring batch read` 的语义归档。当前代码已经具备 effective beam width 计算、batch candidate 扩展、批量 page read、io_uring fallback 和相关统计；后续如果继续改搜索循环，应保持这里定义的参数语义。
...
你现在在 `/home/liang/agent-aware` 项目中继续完善磁盘图搜索的 `beam_width + io_uring` 批量读取逻辑。
...
如果项目已有 `io_uring reader`，请优先复用，不要重复造一个完全独立的 reader。
...
检查 `tools/benchmarks/sift_search_benchmark.cpp`：
...
```bash
./build/agentmem_flow --dataset sift --data_root ~/data/sift --search_width 64  --beam_width 1  --topk 10
./build/agentmem_flow --dataset sift --data_root ~/data/sift --search_width 64  --beam_width 4  --topk 10
./build/agentmem_flow --dataset sift --data_root ~/data/sift --search_width 64  --beam_width 8  --topk 10
./build/agentmem_flow --dataset sift --data_root ~/data/sift --search_width 64  --beam_width 16 --topk 10
./build/agentmem_flow --dataset sift --data_root ~/data/sift --search_width 128 --beam_width 8  --topk 10
./build/agentmem_flow --dataset sift --data_root ~/data/sift --search_width 128 --beam_width 16 --topk 10
./build/agentmem_flow --dataset sift --data_root ~/data/sift --search_width 256 --beam_width 16 --topk 10
```
