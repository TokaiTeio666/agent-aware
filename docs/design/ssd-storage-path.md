...
P2 的核心交付已经进入主路径：项目具备 4KB disk record、磁盘索引读写、packed graph 文件、O_DIRECT/pread 读路径以及对应测试。该文档现在作为 SSD 存储链路的设计归档和验收依据。
...
| 阶段 | 时间 | 核心任务 | 交付物 |
|---|---|---|---|
| P2 SSD 存储链路 | 第 2–3 周 | 完成 4KB 记录布局、`O_DIRECT` 读写、`DiskIndexReader` / `DiskIndexWriter` | SSD index 文件、同步读写测试 |

本阶段目标是把图索引从“内存结构”落盘为“SSD 可随机读取的索引文件”，为后续图搜索、异步 I/O、预取、缓存和 io_uring 优化提供稳定的磁盘访问基础。
...
tests/unit/storage/
  test_disk_record.cpp
  test_disk_index_rw.cpp
```
...
测试文件：`tests/unit/storage/test_disk_record.cpp`
...
测试文件：`tests/unit/storage/test_disk_index_rw.cpp`
...
可选扩展：如需单独压测同步磁盘读，可在 `tools/benchmarks/` 下新增独立 benchmark。
...
add_executable(test_disk_record tests/unit/storage/test_disk_record.cpp)
target_link_libraries(test_disk_record PRIVATE storage)

add_executable(test_disk_index_rw tests/unit/storage/test_disk_index_rw.cpp)
target_link_libraries(test_disk_index_rw PRIVATE storage)

# 可选：add_executable(bench_sync_disk_read tools/benchmarks/bench_sync_disk_read.cpp)
