...
std::shared_ptr<agentmem::dynamic::SSTableReader> write_and_open(
  ...
  auto reader =
      std::make_shared<agentmem::dynamic::SSTableReader>(base_path(dir, id));
...
void test_two_sstables_merge_to_one() {
  ...
  const auto result = agentmem::dynamic::CompactionJob(input).run();
  ...
  agentmem::dynamic::SSTableReader output(result.output_base_path);
