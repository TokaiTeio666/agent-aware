...
void test_write_generates_files_and_reopen_get() {
  ...
  agent_aware::dynamic::SSTableReader reader(base);
  ...
  require(out.neighbors == std::vector<agent_aware::NodeId>({11, 12}),
          "get neighbors");
}

void test_get_missing_returns_false() {
  const auto dir = test_dir("sstable_missing");
  agent_aware::dynamic::SSTableWriter writer(dir, 2, 0);
  require(writer.write({make_record(1, 1, {1.0f})}), "write missing test");

  agent_aware::dynamic::SSTableReader reader(base_path(dir, 2));
...
void test_duplicate_node_id_keeps_newest_sequence() {
  ...
  std::vector<agent_aware::DynamicRecord> records = {
  ...
  agent_aware::dynamic::SSTableWriter writer(dir, 3, 1);
  require(writer.write(records), "write duplicate records");

  agent_aware::dynamic::SSTableReader reader(base_path(dir, 3));
...
void test_scan_all_returns_all_latest_records() {
  ...
  agent_aware::dynamic::SSTableWriter writer(dir, 4, 0);
  ...
  agent_aware::dynamic::SSTableReader reader(base_path(dir, 4));
...
void test_reader_accepts_file_path_as_base() {
  ...
  agent_aware::dynamic::SSTableWriter writer(dir, 5, 2);
  require(writer.write({make_record(1, 5, {5.0f})}), "write file path test");

  agent_aware::dynamic::SSTableReader reader(base_path(dir, 5).string() + ".data");
